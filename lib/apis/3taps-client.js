...
threeTapsNotificationsClient.prototype = {
	 ...
	 *     <tr>
	 *         <td>filter</td>
	 *         <td>List of filter criteria to use to select which postings to send out notifications for. The filter should be an array, where each entry in the array is an object with the following fields:
	 * 
	 *             <table>
	 *                 <tr>
	 *                     <td>type</td>
	 *                     <td>A string identifying the type of filter to create.  The following filter types are currently supported:
	 *
     *                         category
	 *                         location
	 *                         source
	 ...
	 *                 <tr>
	 *                     <td>value</td>
	 *                     <td>A string containing the value the posting must have if it is to match this filter criteria.  For category, location and source, the value must be the appropriate 3taps code for the category, location and source respectively.  For the other filter criteria, the value can be any freeform text.
	 *                 </tr>
	 *             </table>
	 *         </td>
	 *     </tr>
	 *     <tr>
	 *         <td>format</td>
	 *         <td>is a string defining how the notifications should be formatted.  The following formats are currently supported:
	 *
	 *             <table>
	 *                 <tr>
	 *                     <td>brief</td>
	 ...
	 *     <tr>
	 *         <td>destination</td>
	 *         <td>A string defining how the notifications will be sent out.  The following destinations are currently supported:
	 * 
	 *             email
	 *             twitter
	 *             xmpp</td>
	 ...
	 *     <tr>
	 *         <td>destInfo</td>
	 *         <td>An object containing additional information needed to send the notifications to a given destination.  The fields required in this object will vary depending on the destination:
	 * 
	 *             <table>
	 *                 <tr>
	 *                     <td>email</td>
	 *                     <td>For notification requests sent out via email, the destInfo object should include the following fields:
	 * 
	 *                         emailAddress
	 *                         subject
	 * 
	 *                         Note that the email address to send the notifications to is required, while the subject is optional.</td>
	 *                 </tr>
	 *                 <tr>
	 *                     <td>twitter</td>
	 *                     <td>For notification requests via Twitter, the destInfo object should include the following fields:
	 * 
	 *                         username
	 *                         password
	 * 
	 *                         These define the Twitter account to use to create the notification tweets.  Both fields must be supplied.</td>
	 *                  </tr>
	 *                  <tr>
	 *                      <td>xmpp</td>
	 *                      <td>For notification requests via XMPP, the destInfo object should include the following fields:
	 * 
	 *                          jid
	 *                          username
	 *                          password
	 * 
	 *                          jid defines the Jabber (XMPP) account to send notifications to, while username and password are used to sign in to the XMPP server.</td>
	 *                      </td>
	 *                  </tr>
	 *              </table>
	 *         </td>
	 ...
	 *     <tr>
	 *         <td>batchSize</td>
	 *          <td>An integer defining how many postings to send out at once.  If this is set to 1, notifications will be sent out as soon as the postings are received rather than being batched.</td>
	 *     </tr>
	 *     <tr>
	 *         <td>maxPerHour</td>
	 *         <td>An integer defining the maximum number of notifications (batched or individual) that should be sent out in any given hour.  Note that this number cannot exceed the maximum value specified by the destination itself; if you attempt to send out more notifications than is supported by the destination, the notification request will be rejected.</td>
	 *     </tr>
	 *     <tr>
	 *         <td>maxPerDay</td>
	 *         <td>An integer defining the maximum number of notifications (batched or individual) that should be sent out in any given 24-hour period.  Note that this number cannot exceed the maximum value specified by the destination itself; if you attempt to send out more notifications than is supported by the destination, the notification request will be rejected.</td>
	 *     </tr>
	 * </table>
	 ...
	 * 		subject: '3taps Notification'
	 * 	},
	 * 	batchSize: 1,
	 * 	maxPerHour: 1,
	 * 	maxPerDay: 1
	 * };
	 * threeTapsNotificationsClient.request(JSON.stringify(request), callback);
	...
	request: function(request, callback) {
		...
		return this.client.request(this.path, 'request', null, params, function(results) {
			callback(results);
		});
	}
};
