...
threeTapsPostingClient.prototype = {
	 ...
	 * @desc The exists request queries the system to see if postings exist in the database, based on their source and external ID. This method is DEPRECATED. The Status API should be used instead.
   ...
   *		 "error": "Heading cannot be null"
	 ...
	 * @return {Object} A JSON encoded object with one boolean field called "success".
	 ...
	 * @desc Returns errors found when trying to process the given posting. This method is DEPRECATED. The Status API should be used instead.
	 ...
	 * 
	 * source is a 5-character code identifying where this posting came from.
	 * 
	 * category is a 4-character code identifying the search category to associate with this posting.
	 * 
	 * location is a 3-character code identifying the location at which this post resides. If the posting does not have a location, an empty string should be supplied.
	 * 
	 * latitude is a floating-point number representing the real or estimated GPS latitude for this posting, in decimal degrees.
	 * 
	 * longitude is a floating-point number representing the real or estimated GPS longitude for this posting, in decimal degrees.
	 * 
	 * language is a two-character code identifying the language. If supplied, this must match one of the valid ISO 693-1 language codes.
	 * 
	 * heading is a short (up to 255-character long) piece of text that summarizes the posting.
	 * 
	 * body is the body text to use for this posting, up to 5,000 characters long.
	 * 
	 * price is the optional price to associate with this posting, as a floating-point number.
	 * 
	 * currency is the three-letter code identifying which currency the price is in. If supplied, this must match one of the valid ISO 4217 currency codes
	 * 
	 ...
	 * 
	 * accountName is a string containing the account name of the author of the posting in the originating system.
	 * 
	 * accountID is a string containing the account ID of the author of the posting in the originating system. In some cases, this will be the same as accountName.
	 * 
	 * timestamp the date and time at which this posting was made, as a string in YYYY/MM/DD HH:MM:SS format, in UTC.
	 * 
	 * expiration the date and time at which this posting should be deleted from the system, as a string in YYYY/MM/DD HH:MM:SS format, in UTC. If no expiration date/time value is specified, the system will calculate a default expiration date based on the posting’s source.
	 * 
	 * externalID can be any text that identifies this posting in an external system.
	 * 
   ...
   *   "expiration":null
	 ...
	 * @param {Array} postings a JSON-encoded array of JSON objects, one for each posting to be sent to 3taps.
