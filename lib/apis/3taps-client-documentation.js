	...
	for (var type in clients) {
		...
		for (var prop in client.prototype) {
			...
			category.methods[prop] = {
				...
				,exampleCode: (method.example.length > 0) ? utils.trim(method.example[1].desc) : ''
				,exampleResponse: (method.example.length > 1) ? utils.trim(method.example[0].desc) : ''
				,params: method.params || ''
				,returns: method.returns || ''
			};
			
			var restStructure = method.comment.getTag('restStructure');
			if (restStructure.length > 0) category.methods[prop].restStructure = restStructure[0].desc;
			var restUrlExample = method.comment.getTag('restUrlExample');
